...
import gamingGears from "@/data/gamingGearsData.js";

export default function GamingItems() {
  ...
  const items = gamingGears?.[category];
  ...
  return (
    <div className="pt-40 px-6 pb-10 text-white flex flex-col items-center">
      ...
      {/* TITLE */}
      <h2 className="text-5xl font-heading-alt mb-8">
        Gaming Gear
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-16">
        {items.map((item) => (
          <div
            key={item.id}
            onClick={() =>
              navigate(`/gaming/${category}/${item.id}`)
            }
          ...
          >
            {/* IMAGE */}
            <div
              className="h-48 bg-gradient-to-br from-gray-800 to-gray-900 
